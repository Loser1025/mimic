...
- **既存ファイルの変更は必ず `edit_file` で差分編集する。`write_file` は新規ファイル作成のみに使う**
        ...
        if self.long_term_memory and self.rotator.accounts:
            try:
                ...
                if _lessons:
                    ...
                    # 注入量の上限: 教訓3件, セッション2件 (簡易的に上位5件まで)
                    # 本来は型別に分けるが、RRFの結果順に抽出
...
class InteractiveOrchestrator:
    ...
    def _next_embed_key(self) -> str:
        """embedding 用アカウントをラウンドロビンで返す"""
        accounts = self.agent.rotator.accounts
        if not accounts:
            return ""
        key = accounts[self._embed_idx % len(accounts)].api_key
        self._embed_idx += 1
