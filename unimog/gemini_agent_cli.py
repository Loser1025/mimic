...
def _rich_print_status(console: "Console", agent: "GeminiAgent"):
    """Rich テーブルでアカウントステータスを表示"""
    status = agent.rotator.status()
    table = Table(
        title="◈  Account Status",
        border_style=RAZER_DIM,
        header_style=f"bold {RAZER_GREEN}",
        title_style=f"bold {RAZER_GREEN}",
    ...
    for s in status:
        tokens    = s["tokens_available"]
        rpm_limit = s["rpm_limit"]
        ratio     = tokens / max(rpm_limit, 1)
        color     = RAZER_GREEN if ratio > 0.5 else ("yellow" if ratio > 0.2 else "red")
        filled    = int(tokens)
        bar       = "█" * filled + "░" * (rpm_limit - filled)
        rpd       = "∞" if s["rpd_unlimited"] else str(s["rpd_remaining"])
        model_short = s["model"].replace("gemini-", "").replace("gemma-", "gemma/")
        table.add_row(
            s["name"],
            model_short,
            f"[{color}]{tokens:.1f}/{rpm_limit}[/]",
            rpd,
            f"[{color}]{bar}[/]",
        )
    console.print(table)
...
def interactive_loop(agent: GeminiAgent):
    ...
    if not RICH_AVAILABLE:
        # fallback: rich 未インストール時
        print("=" * 60)
        print("  Gemini AIエージェント  (終了: exit / quit / Ctrl+C)")
        print("  pip install rich でリッチ UI が有効になります")
        print("=" * 60)
        _interactive_loop_plain(agent)
        return

    console = Console(highlight=False)

    # stdout に流れる JSON ログを非表示にしてターミナルをクリーンに保つ
    ...
    banner = Text.assemble(
        ("◈ ", f"bold {RAZER_GREEN}"),
        ("UNIMOG", "bold white"),
        ("  ·  Gemini Agent", "dim white"),
    )
    console.print(Panel(banner, border_style=RAZER_GREEN, padding=(0, 2)))
    console.print(f"  作業フォルダ: [dim]{agent.cwd}[/dim]")
    console.print(f"  [dim]/help でコマンド一覧  ·  exit で終了[/dim]\n")

    # ── 入力ループ ─────────────────────────────────────────────
    while True:
        try:
            user_input = console.input(f"[bold {RAZER_GREEN}]❯ [/]").strip()
        ...
        if user_input.lower() in ("exit", "quit", "q"):
            console.print(f"[{RAZER_GREEN}]◈[/] 終了します。")
            break

        # /status は Rich テーブルで上書き
        if user_input.strip() == "/status":
            _rich_print_status(console, agent)
            continue
        ...
        try:
            with console.status(
                Text(" 思考中...", style=f"bold {RAZER_GREEN}"),
                spinner="dots",
                spinner_style=f"bold {RAZER_GREEN}",
            ):
...
def _interactive_loop_plain(agent: GeminiAgent):
    """Rich 未インストール時のフォールバック"""
    while True:
        try:
            user_input = input("\n>>> ").strip()
        except (EOFError, KeyboardInterrupt):
