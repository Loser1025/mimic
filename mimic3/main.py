...
from .orchestrator import InteractiveOrchestrator, AgentOrchestrator, DualModelOrchestrator
...
def interactive_loop(
    ...
    react_prompt: str,
    plan_prompt: str,
    dual_orch: "Optional[DualModelOrchestrator]" = None,
):
    """
    ハイブリッドエージェントのメインループ。
    ...
    current_mode = "dual" if dual_orch is not None else "interactive"

    def _mode_label(mode: str) -> str:
        if mode == "interactive":
            return C.bold_green("Interactive (ReAct)")
        if mode == "dual":
            return C.cyan("Dual (Thinker + Actor)")
    ...
    def switch_mode(new_mode: str):
        ...
        agent.set_system_prompt(react_prompt if new_mode != "plan" else plan_prompt)
        agent.clear_history()
    ...
    @cmd_registry.register("mode", "モード切り替え (/mode interactive|plan|dual)")
    def cmd_mode(a: "OpenRouterAgent", args: str):
        ...
        if m in ("interactive", "react", "i"):
            ...
            if dual_orch is None:
                safe_print(C.red("  dual モードは MISTRAL_API_KEY が設定されている場合のみ使用できます。"))
            else:
                switch_mode("dual")
        else:
            safe_print(f"  現在: {_mode_label(current_mode)}")
            dual_note = "  /mode dual     — Thinker(Mistral) + Actor(OwlAlpha)\n" if dual_orch else ""
            safe_print(C.gray(
                f"  切替: /mode interactive  /mode plan\n{dual_note}"
                "  ※ dual は .env に MISTRAL_API_KEY が必要"
            ))
    ...
    def cmd_autoexport(a: "OpenRouterAgent", args: str):
    ...
    safe_print(C.gray(
        "  /mode dual|interactive|plan でモード切り替え  "
        "·  /help で一覧  ·  exit で終了"
    ) + "\n")
    ...
    while True:
        ...
        # ── モードに応じたルーティング ──────────────────────────
        if current_mode == "dual" and dual_orch is not None:
            try:
                safe_print(f"\n{C.green_dim('┌─')} {C.cyan('mimic [dual]')}")
                dual_orch.run(user_input)
                safe_print(C.green_dim("└" + "─" * 20) + "\n")
            except KeyboardInterrupt:
                safe_print(C.yellow("\n\n  [割り込み] Ctrl+C"), flush=True)
            except Exception as e:
                safe_print(C.red(f"\n  ✗ {e}"))
                log.error({"event": "dual_error", "error": str(e),
