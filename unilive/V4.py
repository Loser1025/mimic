...
def _try_read_file_text(path: Path) -> str:
    """
    エンコーディングを utf-8 -> cp932 -> euc-jp の順で試行してファイルを読み込む。
    すべて失敗した場合は errors="replace" で強制的に読み込む。
    """
...
def search_files(pattern: str, path: str = ".", glob_pattern: str = "**/*",
    ...
    for file_path in sorted(search_path.glob(glob_pattern)):
        if not file_path.is_file():
            continue
        try:
            content = _try_read_file_text(file_path)
...
def ask_user(question: str) -> str:
    safe_print(C.bold_purple(f"\n  [AIからの質問]"))
    safe_print(C.purple(f"  {question}"))
    try:
...
class AutoGit:
    ...
    def _has_changes(self, cwd: str) -> bool:
        ...
        if rc != 0:
            return False
        # untracked files（?? 行）はバックアップ対象外：変更済みファイルのみ検出
...
def get_embedding(text: str, api_key: str) -> list:
    ...
    url = (
        ...
        f"gemini-embedding-2-preview:embedContent?key={api_key}"
    )
    # model フィールドは URL で指定済みのためボディには不要
    # gemini-embedding-2-preview は 8,192 トークンのコンテキストに対応（約 30,000 文字）
    payload = json.dumps({
...
# ──────────────────────────────────────────────
# LongTermMemory（軽量 RAG 長期記憶）
# ──────────────────────────────────────────────

# #7: TTL — 作成から N 日以上 & use_count==0 の教訓を自動廃棄
_LESSON_TTL_DAYS = 30
# #4: 重複判定の類似度閾値（これ以上類似なら同一教訓とみなしてスキップ）
_LESSON_MERGE_THRESHOLD = 0.92
...
class LongTermMemory:
    ...
    def _save(self):
        """
        embedding 配列だけ1行コンパクト形式で保存し、
        それ以外のフィールドは読みやすく indent=2 で出力する。
        3072次元のベクトルを行ごとに展開すると数千行になるのを防ぐ。
        """
        try:
            parts = []
            for entry in self._lessons:
                ...
                # rest を indent=2 でシリアライズ → 末尾 } の直前に embedding を挿入
    ...
    def _expire_old_lessons(self):
        """作成から _LESSON_TTL_DAYS 日以上 & use_count==0 の教訓を削除する"""
        ...
        self._lessons = [
            ...
            if not (
                e.get("use_count", 0) == 0
                and _parse_dt(e.get("created_at", "")) < cutoff
            )
        ]
        removed = before - len(self._lessons)
        if removed:
            self._save()
            safe_print(C.mem(f"  🗑  期限切れ教訓 {removed} 件を削除しました"), flush=True)
            log.info({"event": "lessons_expired", "removed": removed})
    ...
    def add_lesson(self, task_query: str, lesson_text: str, api_key: str):
        """タスクと教訓をベクトル化してDBに追記する（重複はスキップ #4）"""
        # gemini-embedding-2-preview の推奨: タスクタイプをプレフィックスとして付与
        combined  = f"task: retrieval_document: {task_query} {lesson_text}"
        embedding = self._get_embedding_cached(combined, api_key)  # #2

        # #4: 重複チェック（embedding がある場合のみ）
        if embedding and self._is_duplicate(embedding):
            log.info({"event": "lesson_duplicate_skipped",
                      "lesson_preview": lesson_text[:60]})
            safe_print(C.mem("  💡 類似教訓が既に存在するためスキップしました"), flush=True)
            return

        tags = _detect_tags(f"{task_query} {lesson_text}")  # #5
        ...
        self._save()
        log.info({"event": "lesson_added", "tags": tags,
                  "lesson_preview": lesson_text[:80]})
    ...
    def retrieve_relevant_lessons(
        ...
        # #2: キャッシュ付き Embedding (推奨プレフィックス付与)
        query_vec = self._get_embedding_cached(f"task: retrieval_query: {current_task}", api_key)
        ...
        if query_vec:
            ...
            # #1: TF-IDF フォールバック
            log.info({"event": "rag_tfidf_fallback"})
            scored = self._tfidf_retrieve(current_task, candidates, threshold)
...
class GeminiAgent:
    ...
    def _compact_if_needed(self):
        ...
        safe_print(C.gray(f"  [会話要約圧縮中... {len(to_summarize)}件 / {total_chars:,}文字を要約します]"), flush=True)
        ...
        safe_print(C.gray(f"  [要約完了: {len(to_summarize)}件 → 要約に変換、直近{keep_recent}件保持 ({total_chars:,}→{after_chars:,}文字)]"))
    ...
    def _api_call_with_retry(self, messages: list[dict],
        ...
        while attempt < MAX_RETRIES:
            ...
            try:
                ...
                if len(working_messages) > 6 and trim_count < 3:
    ...
    def _stream_react_call(
        ...
        while attempt < MAX_RETRIES:
            ...
            try:
                ...
                if header_printed:
                    safe_print()  # ストリーム末尾の改行（text_callback 有無を問わず統一）
...
def cmd_think(agent: GeminiAgent, args: str):
    if args.lower() == "on":
        ...
        safe_print(f"  現在の推論モード: {state}  (/think on|off で切替)")
...
def interactive_loop(
    ...
    while True:
        ...
        if current_mode == "interactive":
            try:
                result = interactive_orch.run_react(user_input)
...
class AgentOrchestrator:
    ...
    def _execute_steps_parallel(
        ...
        safe_print(C.orange(f"  ⚡ {n} ステップを並列実行"), flush=True)
        ...
        with ThreadPoolExecutor(max_workers=n) as pool:
            futures = {
                pool.submit(
                    ...
                    None, None,  # reviewer_pool=None, on_token=None (並列時はストリーム無効)
                ): step
    ...
    def _reflect_and_correct(
        ...
        final_result: str,
        steps: list["PlanStep"],
        injected_lessons: list[str] = None,
        on_token=None,
    ...
    def run_with_plan(
        ...
        if self.long_term_memory and self.rotator.accounts:
            try:
                _api_key = self.rotator.accounts[0].api_key
                _lessons = self.long_term_memory.retrieve_relevant_lessons(
                    user_message, _api_key
                )
                if _lessons:
                    safe_print(C.bold_mem(
                        f"  🧠 長期記憶から {len(_lessons)} 件の教訓をロードしました"
                    ), flush=True)
                    # #6: 各教訓を最大180文字に圧縮してから表示・注入
                    _compressed = [_compress_lesson(e.get("lesson", "")) for e in _lessons]
                    injected_lesson_texts = [e.get("lesson", "") for e in _lessons]
                    for _l in _compressed:
                        safe_print(C.mem(f"     • {_l}"), flush=True)
                    lessons_block = "\n".join(f"- {l}" for l in _compressed)
                    user_message = (
                        f"[過去の類似タスクからの教訓]\n{lessons_block}\n\n"
                        + user_message
                    )
...
class InteractiveOrchestrator:
    ...
    def _confirm_write_tool(self, fn_name: str, fn_args: dict) -> Optional[str]:
        ...
        """
        _print_write_diff(fn_name, fn_args)
        safe_print(C.yellow(f"  [書き込み確認] {fn_name} を実行しますか？"), flush=True)
        safe_print(C.gray("    y または Enter → 実行"), flush=True)
        safe_print(C.gray("    n             → スキップ"), flush=True)
        try:
    ...
    def _execute_with_intervention(
        ...
        if fn_name in ("write_file", "edit_file", "patch_file"):
            ...
            safe_print(C.gray("    y または Enter → 削除実行"), flush=True)
            safe_print(C.gray("    n             → スキップ"), flush=True)
            try:
                ans = input(C.bold_green("  >>> ")).strip().lower()
            except (EOFError, KeyboardInterrupt):
        ...
        while True:
            try:
                ...
                safe_print(C.gray("  どうしますか？"))
                safe_print(C.gray("    r       → リトライ"))
                safe_print(C.gray("    c       → このツールをスキップして続行"))
                safe_print(C.gray("    その他  → 指示を入力してAIに渡す"))
                try:
    ...
    def run_react(self, user_message: str) -> str:
        ...
        if self.long_term_memory and self.agent.rotator.accounts:
            try:
                _api_key = self.agent.rotator.accounts[0].api_key
                _lessons = self.long_term_memory.retrieve_relevant_lessons(
                    user_message, _api_key
                )
                if _lessons:
        ...
        while step_count < self.MAX_REACT_STEPS:
            # text_callback=None → トークンをリアルタイムで端末に表示（💭 ストリーミング）
            ...
            if not tool_calls:
                ...
                # ── Eval: フィードバック適用 & 教訓抽出 ──────────────
                if self.long_term_memory and self.agent.rotator.accounts and step_count > 0:
                    try:
                        _api_key = self.agent.rotator.accounts[0].api_key
                        
                        # 注入された教訓がある場合、その有効性を判定させる
                        helpful, unhelpful = [], []
                        if injected_lesson_texts:
                            list_str = "\n".join(f"- {l}" for l in injected_lesson_texts)
                            eval_raw = self.agent.run(
                                f"[タスク]\n{user_message[:500]}\n\n"
                                f"[最終回答]\n{final_text[:1000]}\n\n"
                                f"[注入された教訓]\n{list_str}\n\n"
                                "上記の各教訓が、今回の回答を導くのに「実際に役立ったか」を判定し、"
                            ...
                            if m:
                                try:
                                    ...
                                    helpful = d.get("helpful", [])
                                    unhelpful = d.get("unhelpful", [])
                                except: pass
                        ...
                        if lesson_text and lesson_text not in ("なし", "なし。"):
                            self.long_term_memory.add_lesson(
                                user_message, lesson_text, _api_key
                            )
                            preview = lesson_text[:80] + ("..." if len(lesson_text) > 80 else "")
                            safe_print(C.bold_mem(f"  💡 新しい教訓を記憶しました"), flush=True)
                            safe_print(C.mem(f"     {preview}"), flush=True)
                    except Exception as _e:
                        log.warning({"event": "react_eval_error", "error": str(_e)})
...
def main():
    ...
    def cmd_plan(agent: GeminiAgent, args: str):
        if not args:
            safe_print("  使い方: /plan <タスク>")
            safe_print("  ヒント: /mode plan で Plan モードに固定切り替えも可能")
            return
    ...
    def cmd_reviewer(agent: GeminiAgent, args: str):
        ...
        if a in ("off", "0", "false"):
            ...
            safe_print(C.yellow("  Reviewer: OFF（ステップ検証をスキップ・高速モード）"))
        elif a in ("on", "1", "true"):
            orchestrator.use_reviewer = True
            safe_print(C.green("  Reviewer: ON（ステップ検証を実行・高精度モード）"))
        else:
            state = C.green("ON") if getattr(orchestrator, "use_reviewer", True) else C.yellow("OFF")
            safe_print(f"  Reviewer: {state}  (/reviewer on|off で切り替え)")
    ...
    if "--prompt" in args:
        ...
        if idx + 1 < len(args):
            ...
            safe_print("--prompt の後に質問文を指定してください。")
            sys.exit(1)
    elif "--status" in args:
        agent.print_status()
        sys.exit(0)
