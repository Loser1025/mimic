...
def split_audio(audio_path: str, chunk_length_min: int = CHUNK_LENGTH_MIN) -> list[str]:
    ...
    for i, start_ms in enumerate(range(0, len(audio), chunk_length_ms)):
        ...
        if len(chunk) < 500:  # 500ms未満は空チャンクとしてスキップ
            print(f"   チャンク {i+1} をスキップ（{len(chunk)}ms、短すぎるため）")
            continue
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                # retry-after ヘッダーから待機時間を取得（存在する場合）
                wait_time = retry_delay
                try:
                    if hasattr(e, 'response') and e.response is not None:
                        ra = (e.response.headers.get('retry-after') or
                              e.response.headers.get('x-ratelimit-reset-requests'))
                        if ra:
                            wait_time = float(ra) + 5
                except Exception:
                    pass
                print(f"  ⚠️ レート制限に達しました。{wait_time:.0f}秒待機してリトライします... ({attempt + 1}/{max_retries})")
                time.sleep(wait_time)
                retry_delay *= 2  # 指数バックオフ（次回のフォールバック用）
            else:
...
def main():
    ...
    parser.add_argument("--resume-from", type=int, default=1,
                        help="このチャンク番号から処理を再開する（1始まり、デフォルト: 1）")
    ...
    resume_from = args.resume_from - 1  # 0-indexed
    for i, chunk_path in enumerate(chunk_files):
        if i < resume_from:
            print(f"⏭️  チャンク {i+1} をスキップ（--resume-from {args.resume_from} のため）")
            transcript_parts.append(f"<!-- チャンク{i+1} スキップ -->")
            continue
        print(f"\n{'='*50}")
